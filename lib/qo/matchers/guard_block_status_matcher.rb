module Qo
  module Matchers
    ...
    # @since 0.1.5
    #
    class GuardBlockStatusMatcher < BaseMatcher
      ...
      def initialize(status, array_matchers, &fn)
        @fn     = fn || IDENTITY
        @status = status

        super('and', *array_matchers)
      end
      ...
      def to_proc
        Proc.new { |target|
          target[0] == @status && super[target[1..-1]] ? [true, @fn.call(target[1..-1])] : NON_MATCH
        }
      end
    end
  end
