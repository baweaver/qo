module Qo
  module Matchers
    ...
    # @since 0.5.0
    #
    class TypedGuardBlockMatcher < BaseMatcher
      # Definition of a non-match
      NON_MATCH = [false, false]

      def initialize(guarantee, array_matchers, keyword_matchers, &fn)
        @fn = fn || Qo::IDENTITY
        @guarantee = guarantee
      ...
      def call(target)
        return false unless @guarantee === target[0]

        if super(target.last)
          [true, @fn.call(target.last)]
        else
          NON_MATCH
        end
      end
